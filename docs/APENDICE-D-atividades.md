...
(já publicada, sem aprovação prévia do administrador), até a execução e conclusão da coleta pela
empresa. O administrador só intervém reativamente, removendo solicitações identificadas como abuso.
...
- A solicitação **nasce** com `status = aprovada` — não há fila de aprovação prévia pelo administrador.
- A solicitação fica visível para empresas enquanto `status = aprovada` **e** ainda não possui coleta.
- O administrador pode remover reativamente (`status = removida`) uma solicitação já publicada, a
  qualquer momento, caso identifique abuso.
