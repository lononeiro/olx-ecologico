...
export function Navbar({ mobileOpen = false, onClose }: NavbarProps) {
  ...
  const links: Record<string, { href: string; label: string; icon: React.ReactNode }[]> = {
    usuario: [
      ...
      { href: "/dashboard/solicitacoes", label: "Solicitacoes", icon: <IconList /> },
      { href: "/me", label: "Meu Perfil", icon: <IconUser /> },
    ],
    admin: [
      { href: "/admin", label: "Painel", icon: <IconGrid /> },
      { href: "/admin/solicitacoes", label: "Aprovacoes", icon: <IconCheck /> },
      { href: "/me", label: "Meu Perfil", icon: <IconUser /> },
    ],
    empresa: [
      { href: "/empresa", label: "Painel", icon: <IconGrid /> },
      { href: "/empresa/solicitacoes", label: "Disponiveis", icon: <IconList /> },
      ...
      >
        ...
        >
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  ...
                  onMouseOver={(event) => {
                    if (!active) event.currentTarget.style.background = "var(--surface-2)";
                  }}
                  onMouseOut={(event) => {
          ...
          {session ? (
            <div
              className="hide-mobile"
              style={{ display: "flex", alignItems: "center", gap: ".65rem", flexShrink: 0 }}
            >
              <ThemeToggle compact />
              <Link
                href="/me"
...
function RolePill({ role }: { role: string }) {
  const map: Record<string, { label: string; color: string; bg: string }> = {
    usuario: { label: "Cidadao", color: "var(--green)", bg: "rgba(30,122,50,.1)" },
