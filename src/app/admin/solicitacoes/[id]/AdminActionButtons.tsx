...
export function AdminActionButtons({ solicitacaoId }: { solicitacaoId: number }) {
  ...
  async function handleAction(aprovado: boolean) {
    ...
    setLoading(null);
    if (res.ok) {
      setResultado({ ok: true, msg: aprovado ? "Solicitação aprovada com sucesso." : "Solicitação rejeitada." });
    ...
    return (
      <div style={{
        ...
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
          {resultado.ok ? <path d="M20 6 9 17l-5-5"/> : <><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></>}
        </svg>
        {resultado.msg}
      </div>
    );
